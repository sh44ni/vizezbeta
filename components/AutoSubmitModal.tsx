...
export default function AutoSubmitModal({ isOpen, onClose, applicants, portalId }: AutoSubmitModalProps) {
  ...
  const downloadPdf = () => {
    ...
    // Format the applicant name: e.g. "SOMA PRAJAPATI" -> "SOMA_PRAJAPATI"
    const applicantName = currentApplicant?.data?._name ? currentApplicant.data._name.replace(/\s+/g, '_') : 'APPLICANT';
  ...
  const downloadPhotoPdf = async () => {
    const photoFile = currentApplicant?.data?._photoFile;
    if (!photoFile) {
      alert('No photo was provided for this applicant.');
      return;
    }
    ...
    const photoDataUrl = await fileToDataUrl(photoFile as File);
    ...
    // Photo dimensions from Photoshop: W: 1.940 in, H: 2.263 in
    const width = 1.940;
    const height = 2.263;
    const startX = 0.5;
    const startY = 0.5;
        ...
        {status === 'success' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              {result?.pdfBase64 ? (
                <button className="btn-ghost" onClick={downloadPdf} style={{ flex: 1, padding: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  <Download className="w-4 h-4" /> Application PDF
                </button>
              ) : (
                <div style={{ flex: 1, fontSize: '12px', color: 'var(--warning)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ⚠️ Application PDF unavailable.
                </div>
              )}

              {currentApplicant?.data?._photoFile && (
                <button className="btn-ghost" onClick={downloadPhotoPdf} style={{ flex: 1, padding: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  <Download className="w-4 h-4" /> Photos PDF
                </button>
              )}
            </div>
