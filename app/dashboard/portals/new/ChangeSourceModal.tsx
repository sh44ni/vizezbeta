...
import { mockSourceFields } from '../portal-mock-data';
...
export default function ChangeSourceModal({ currentLabel, onSelect, onClose }: Props) {
  ...
  const filtered = mockSourceFields.filter(s =>
          ...
          {filtered.map(s => (
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{s.value}</div>
              </div>
